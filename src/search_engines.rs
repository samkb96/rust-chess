...
use crate::engine_handler::{ SearchEngine, Evaluator, Evaluation, SearchData };
use rand::prelude::*;
use std::thread;
...
pub struct Negamax { pub depth: u8 }
pub struct AlphaBetaPruning { pub depth: u8 }

// -------------------------------------------------------- version 0 --------------------------------------------------------
impl SearchEngine for RandomSearch {
    fn search(&self, search_state: &mut GameState, _side_to_move: PieceColour, _search_data: &mut SearchData, _evaluator: &dyn Evaluator) -> Option<Move> {
...
impl SearchEngine for Negamax {
    fn search(
        ...
        for mv in legal_moves {
            ...
            let score = -Negamax::negamax(search_state, &side_to_move, depth.saturating_sub(1), search_data, evaluator);
...
impl SearchEngine for AlphaBetaPruning {
    fn search(
        ...
        let mut alpha: Evaluation = i32::MIN;
        let mut beta: Evaluation = i32::MAX;
...
fn evaluate_end_of_game(search_state: &GameState, legal_moves: &Moves, side_to_move: &PieceColour) -> Option<Evaluation> {
    if let Some(game_ending) = search_state.game_is_over(side_to_move, legal_moves) {
        match (side_to_move, game_ending) {
            (PieceColour::White, GameEnding::WhiteWins) | (PieceColour::Black, GameEnding:: BlackWins) => Some(1000000),
            (PieceColour::White, GameEnding::BlackWins) | (PieceColour::Black, GameEnding:: WhiteWins) => Some(-1000000),
            _ => Some(0) // draws
        }
    } else { 
