...
use crate::search_engines::{RandomSearch, Negamax};
...
impl BotVersion {
    pub fn to_bot(&self) -> Bot {
        ...
        match self {
            BV::Random => Bot { search_engine: Box::new(RandomSearch), evaluator: Box::new(NullEvaluator)},
            BV::Negamax => Bot { search_engine: Box::new(Negamax { depth: 3 }), evaluator: Box::new(PieceValues)},
        }
    }
}
...
pub trait SearchEngine: Send + Sync {
    fn search(&self, search_state: &mut GameState, side_to_move: PieceColour, search_data: &mut SearchData, evaluator: &dyn Evaluator) -> Option<Move>;
}

pub trait Evaluator: Send + Sync {
    fn evaluate(&self, side_to_move: &PieceColour, search_state: &GameState, search_data: &mut SearchData) -> Evaluation;
}
...
impl Bot {
    ...
    pub fn choose_move(&self, game_state: &mut GameState) -> Option<Move> {
        ...
        if let Some(move_choice) = self.search_engine.search(&mut search_state, game_state.side_to_move, &mut search_data, &*self.evaluator) {
